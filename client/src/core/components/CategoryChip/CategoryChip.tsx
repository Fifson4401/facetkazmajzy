...
> = ({ category, isCategory, parentID }) => {
  ...
  const url = isCategory
    ...
    : parentID ? `/zadania?category=${parentID}&subCategory=${category.data.id}` : `/zadania?subCategory=${category.data.id}`;

  const displayName = isCategory ? category.data.attributes.name : getDisplayCategory(category.data.attributes.name)
