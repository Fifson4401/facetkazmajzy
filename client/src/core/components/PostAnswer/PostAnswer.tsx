...
const PostAnswer: FC<BlogPageAnswerProps> = ({ TEX, image }) => {
  return (
    <div className="md:px-11 w-full">
      <Accordion variant="bordered" motionProps={animationVariant} className="w-full border-[#cc3266]">
        <AccordionItem key="1" aria-label="Odpowiedź na zadanie" title="Zobacz odpowiedź!" indicator={<FaAngleDown color="#323232" />}>
          <PostContentRenderer
            key={`TEX_answer`}
            content={TEX}
            className="md:px-4"
